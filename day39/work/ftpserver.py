...
class MyThreadingTCPServer(socketserver.ThreadingTCPServer):
    def verify_request(self, request, client_address):
        ...
        :param client_address: ('127.0.0.1', 45367)
        :return:
        """
        while 1:
            ...
            if username == b'alex' and password == b'123456':
                request.sendall('yes'.encode('utf-8'))

                if not os.path.exists(username.decode('utf-8')):
                    os.makedirs('/root/%s'%username.decode('utf-8'))

                return True
            else:
                request.sendall('no'.encode('utf-8'))
